        ...
        return (
            <View style={styles.pageWrap}> 
                ...
                <ScrollView style={styles.modalContentScroll}>
                    ...
                    <Text style={styles.stepTitle}>STEP1 제휴할인을 선택해주세요.</Text>

                    {/* 3. 버튼: KT, T우주 - Layer 1 (2열) */}
                    <View style={styles.allianceButtonRow}>
                        <AllianceButton
                            name="KT VIP초이스" icon="KT" subText="(통합 월 1회)" methodKey="kt" 
                            selectedMethod={selectedMethod}
                            onPress={() => handlePlaceholderAction('KT VIP초이스', 'kt', setSelectedMethod)}
                        />
                        <AllianceButton
                            name="SKT우주패스" icon="T우주" methodKey="uwoo" 
                            selectedMethod={selectedMethod}
                            onPress={() => handlePlaceholderAction('SKT우주패스', 'uwoo', setSelectedMethod)}
                        />
                    </View>

                    {/* 4. 텍스트: STEP2 결제수단을 선택해주세요 */}
                    <Text style={[styles.stepTitle, styles.mt20]}>STEP2 결제수단을 선택해주세요.</Text>

                    {/* 5. 카드결제, 앱카드 (버튼) - Layer 2 (2열) */}
                    <View style={styles.largePayButtonRow}>
                        <LargePayButton
                            icon="💳" mainText="카드결제" subText="삼성페이/LG페이" methodKey="card"
                            selectedMethod={selectedMethod}
                            onPress={() => 
                                router.push({
                        ...
                        />
                        <LargePayButton
                            icon="📲" mainText="앱카드" subText="QR/바코드" methodKey="app"
                            selectedMethod={selectedMethod}
                            onPress={() => handleCardPayment('app')} // <-- 활성화할 버튼
                        />
                    </View>
                    
                    {/* 6. 간편 결제 버튼 Grid - Layer 3 (4열 Grid) */}
                    <View style={styles.simplePayGrid}>
                        {simplePayButtons.map((item) => (
                            <SimplePayItem
                                key={item.name}
                                name={item.name}
                                icon={item.icon}
                                selectedMethod={selectedMethod}
                                onPress={() => handlePlaceholderAction(item.name, item.methodKey, setSelectedMethod)}
                            />
                        ))}
                    </View>

                    {/* 7. 쿠폰사용, 메가선물페이 (버튼) - Layer 4 (2열) */}
                    <View style={styles.otherButtonRow}>
                        <CouponPayButton
                            icon="🎟️" mainText="쿠폰사용" methodKey="coupon"
                            selectedMethod={selectedMethod}
                            onPress={() => handlePlaceholderAction('쿠폰사용', 'coupon', setSelectedMethod)}
                        />
                        <CouponPayButton
                            icon="🎁" mainText="메가선물페이" methodKey="coupon"
                            selectedMethod={selectedMethod}
                            onPress={() => handlePlaceholderAction('메가선물페이', 'coupon', setSelectedMethod)}
                        />
                    </View>
