...
export default function OrderDetailScreen() {
  ...
  const handleSetSelect = (setType: (typeof SET_TYPES)[0]) => {
    recordMcDonaldsSuccess(); // [추가] 옵션 선택 성공
        ...
        <View style={styles.sidebar}>
          {["세트 선택", "사이드", "음료", "주문 확인"].map((label, index) => {
            ...
            return (
              <View
                key={stepNum}
                style={[styles.sidebarItem, isSkipped && { opacity: 0.3 }]}
              >
                <View
                  style={[
                    styles.stepCircle,
            ...
            <ScrollView contentContainerStyle={styles.grid}>
              {SET_TYPES.map((type) => (
                <Pressable
                  ...
                  style={[
                    styles.card,
                    selectedSetType.id === type.id && styles.cardSelected,
                  ]}
          ...
          <View style={styles.stepContainer}>
            ...
            <ScrollView contentContainerStyle={styles.grid}>
              {SIDE_OPTIONS.map((side) => {
                if (selectedSetType.id === "large" && side.id === "fries")
                  return null;
                let displayPrice = side.price;
                if (selectedSetType.id === "large" && side.id === "fries_large")
                  displayPrice = 0;
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      styles.card,
                      selectedSide.id === side.id && styles.cardSelected,
                    ]}
                    onPress={() => {
                  ...
                  >
                    ...
                    {displayPrice > 0 && (
                      <Text style={styles.optionPrice}>+{displayPrice}원</Text>
                    )}
                  </Pressable>
                );
              })}
            ...
            <ScrollView contentContainerStyle={styles.grid}>
              {DRINK_OPTIONS.map((drink) => (
                <Pressable
                  ...
                  style={[
                    styles.card,
                    selectedDrink.id === drink.id && styles.cardSelected,
                  ]}
                  onPress={() => {
                ...
                >
                  ...
                  {drink.price > 0 && (
                    <Text style={styles.optionPrice}>+{drink.price}원</Text>
                  )}
                </Pressable>
              ))}
            </ScrollView>
        ...
        <View style={styles.footer}>
          <Pressable style={styles.cancelButton} onPress={goBack}>
            <Text style={styles.cancelButtonText}>
              ...
              {!isSetFlow ||
              step === 1 ||
              (step === 4 && selectedSetType.id === "single")
                ? "취소"
                : "이전단계"}
            </Text>
          </Pressable>
